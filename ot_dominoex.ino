...
void ot_dominoex_init()
{
    ...
    /* set pins */
    P2SEL |= BIT0; // Radio data Pin = TA1.0 output     
    
    /* Timer A */
    TA1CCTL1 = OUTMOD_7;                // TOGGLE // TACCR1 reset/set
    TA1CTL = TASSEL_2 + MC_1 + TACLR;   // SMCLK, upmode
    TA1CCR0 = 610;                    // Period (DX16 ~ 840, DX22 ~ 610)

}

void ot_dominoex_reset()
{
   _sym = 0;
   TA1CCR1 = _sym;
} 
...
void ot_dominoex_txsym(uint8_t sym) 
{
    _sym = (_sym + 2 + sym) % 18;
    // write _sym to pwm
    TA1CCR1 = _sym;
    delay(46);
    delayMicroseconds(400); // Symbol delay (DX16 ~ 63.5, DX22 ~ 46.3)
...
{
    for (; *s; s++)
        ot_dominoex_txchar(*s);
}

void ot_dominoex_txid(char* s)
{
    for (; *s; s++)
        ot_dominoex_txchar(*s+256);
}

void ot_dominoex_send_temperature(float t)
{
	int temp;
	char buf[4];
	...
	ot_dominoex_txstring(" C");
}

void ot_dominoex_send_pressure(long p)
{
	int temp;
	char buf[5];
	float t;
